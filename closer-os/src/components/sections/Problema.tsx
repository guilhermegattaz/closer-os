...
function ProblemaItems({ stackRef }: { stackRef: React.RefObject<HTMLDivElement | null> }) {
  return (
...
export function ProblemaCard() {
  ...
  useEffect(() => {
    const refs = [stackRef.current, mobileStackRef.current].filter(Boolean) as HTMLElement[]
    ...
    refs.forEach((stack) => {
      Array.from(stack.querySelectorAll<HTMLElement>('[data-stack]')).forEach((el) => itemObs.observe(el))
    })
      ...
      <div className="s-card">
        <div className="container">
          <div className="problema-grid">
            ...
            {/* Desktop: items inside the card */}
            <div className="problema-right problema-right--desktop">
              <div className="problema-wrap">
                <ProblemaItems stackRef={stackRef} />
              </div>
            </div>
          </div>
        </div>
      </div>
      {/* Mobile: items outside the card */}
      <div className="problema-mobile-items container">
        <ProblemaItems stackRef={mobileStackRef} />
      </div>
    </>
  )
}
